        ...
        for batch in batches:
            async for attempt in AsyncRetrying(
                ...
                with attempt:
                    ...
                    # print(f"Computed embeddings in batch. Batch size: {len(batch.texts)}, Token count: {batch.token_length}")
                    logger.info(
...
class AzureOpenAIEmbeddingService(OpenAIEmbeddings):
    ...
    async def create_client(self) -> AsyncOpenAI:
        ...
        if isinstance(self.credential, AzureKeyCredential):
            ...
            # print(f'Raised an exception in create_client : Invalid credential type : {type(self.credential)}')
