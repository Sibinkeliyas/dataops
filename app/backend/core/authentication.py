...
class AuthenticationHelper:
    ...
    def __init__(
        ...
        # self.valid_audiences = [str(server_app_id)]
    ...
    def build_security_filters(self, overrides: dict[str, Any], auth_claims: dict[str, Any]):
        ...
        prioritize_filter = overrides.get('prioritize_filter',None)
        ...
        if prioritize_filter:
            if 'group_ids' in prioritize_filter:
                group_ids = prioritize_filter['group_ids']
                if isinstance(group_ids,list) and len(group_ids) > 0:
                    priority_filter = "groups/any(g:search.in(g, '{}'))".format(", ".join(group_ids))
            elif 'oids' in prioritize_filter:
                oids = prioritize_filter['oids']
                if isinstance(oids,list) and len(oids) > 0:
                    priority_filter = "oids/any(g:search.in(g, '{}'))".format(", ".join(group_ids))
            else:
                raise AuthError(
                    error="oids and groups must be defined in the search index to use authentication", status_code=400
                )
        ...
        if priority_filter:
            security_filter = f'{priority_filter}'
    ...
    @staticmethod
    async def owned_groups(graph_resource_access_token : dict) -> list[str]:
        headers = {"Authorization": "Bearer " + graph_resource_access_token["access_token"]}
        groups_with_owners = {}
        async with aiohttp.ClientSession(headers=headers) as session:
            resp_json = None
            resp_status = None    
            async with session.get(url="https://graph.microsoft.com/v1.0/me/ownedObjects?$select=id,displayName,mail") as resp:
            ...
            while resp_status == 200:
                ...
                for group in value:
                    groups_with_owners[group['id']] = group
            ...
            if resp_status != 200:
                raise AuthError(error=json.dumps(resp_json), status_code=resp_status)
        return groups_with_owners
    
    @staticmethod
    async def get_combined_group_roles(groups, owned_groups) -> tuple[bool, list[dict]]:
        isAdmin = False
        db_client = current_app.config[config.CONFIG_DB_CLIENT]
  
        db_groups = await db_client.get_groups()

        member_group_ids = groups
        owned_groups_dict = owned_groups

        member_group_ids_set = set(member_group_ids)

        result = []

        # Check if the user is a member of an admin group
        is_admin_member = any(db_group.get('groupType') == 'admin' and db_group.get('entraGroupId') in member_group_ids_set for db_group in db_groups)

        for db_group in db_groups:
            entra_group_id = db_group.get('entraGroupId')
            
            # Include all groups if the user is an admin member
            if is_admin_member or entra_group_id in owned_groups_dict or entra_group_id in member_group_ids_set:
                group_info = {
                    'id': db_group['entraGroupId'],
                    'displayName': db_group.get('groupName'),
                    'highestRole': 'admin' if is_admin_member else 'member',
                    'groupType': db_group.get('groupType', 'default')  # Add groupType
                }
                
                if db_group.get('groupType') == 'admin':
                    group_info['highestRole'] = 'admin'
                    isAdmin = True
                elif entra_group_id in owned_groups_dict and not is_admin_member:
                    group_info['highestRole'] = 'owner'
                    group_info['mail'] = owned_groups_dict[entra_group_id].get('mail')

                if entra_group_id in owned_groups_dict:
                    group_info['displayName'] = group_info['displayName'] or owned_groups_dict[entra_group_id].get('displayName')
    ...
    @staticmethod
    async def list_member_groups(graph_resource_access_token: dict) -> list[str]:
        headers = {"Authorization": "Bearer " + graph_resource_access_token["access_token"]}
        groups = []
        async with aiohttp.ClientSession(headers=headers) as session:
            resp_json = None
            resp_status = None
            async with session.get(url="https://graph.microsoft.com/v1.0/me/memberOf?$select=id") as resp:
                resp_json = await resp.json()
                resp_status = resp.status
                if resp_status != 200:
                    raise AuthError(error=json.dumps(resp_json), status_code=resp_status)
    ...
    async def get_auth_claims_if_enabled(self, headers: dict) -> dict[str, Any]:
        ...
        try:
            ...
            auth_claims['graph_token'] = graph_resource_access_token
...
async def hash_uuid(uuid : str) -> str:
    try:
