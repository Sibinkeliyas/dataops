...
def error_dict(error: Exception,override_error_message : bool = False) -> dict:
    ...
    #     print(error.message)
    #     return {"error": error.message.format(error_type=type(error))}
    # s
    return {"error": ERROR_MESSAGE.format(error_type=type(error))}


def error_response(error: Exception, route: str, status_code: int = 500,override_error_message : bool = False):
    logging.exception("Exception in %s: %s", route, error)
    if isinstance(error, APIError) and error.code == "content_filter":
        status_code = 400
    return jsonify(error_dict(error,override_error_message)), status_code
